...
function radius(x::Array{AbstractFloat,1})
    return x[end]
end

function centerpoint(x::Array{AbstractFloat,1})
    return x[1:end-1]
end
...
test1 = collect(RandomBatches(nf1arr, 20, 1))
test2 = collect(RandomBatches(nf2arr, 20, 1))
test3 = collect(RandomBatches(nf3arr, 20, 1))
test4 = collect(RandomBatches(nf4arr, 20, 1))
